...
* @author tdi
*/
public class ShowOrderXmlAction implements IAction
{
	public ShowOrderXmlAction()
	{
		super();
	}
   ...
   */
	public String execute(HttpServletRequest request, HttpServletResponse response, ArrayList<String> errorList)
	{
		...
		String actionName = request.getParameter(Constants.PARAM_NAME_ACTION);
		...
		if (loginBean != null && loginBean.isLoggedIn())
		{
			...
			if (Security.getInstance().isUserAllowed(loginBean.getUser(), Security.RESOURCE_ALL, Security.ACTION_READ))
			{
				...
				String orderId = request.getParameter(Constants.PARAM_NAME_ORDER_NUMBER);

				if(actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_CUSTOMER_REQUEST_XML) ||
				   actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_CUSTOMER_RESPONSE_XML))
				{
					...
					if(actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_CUSTOMER_REQUEST_XML))
					   xmlContent = new String(boOrderCustomer.getXmlFileRequest());
					else 
					if(actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_CUSTOMER_RESPONSE_XML))
					   xmlContent = new String(boOrderCustomer.getXmlFileResponse());					
				}

				if(actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_PURCHASE_REQUEST_XML) ||
				   actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_PURCHASE_RESPONSE_XML))
				{
					...
					if(actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_PURCHASE_REQUEST_XML))
					   xmlContent = new String(boOrderPurchase.getXmlFileRequest());
					else 
					if(actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_PURCHASE_RESPONSE_XML))
					   xmlContent = new String(boOrderPurchase.getXmlFileRequest());							
				}
				
				request.getSession().setAttribute(Constants.PARAM_XML_TEXT, xmlContent);		
   ...
   */
	...
	{
		return actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_CUSTOMER_REQUEST_XML) ||
			   actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_CUSTOMER_RESPONSE_XML) ||
			   actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_PURCHASE_REQUEST_XML) ||
			   actionName.equalsIgnoreCase(Constants.ACTION_SHOW_ORDER_PURCHASE_RESPONSE_XML);
	}
}
